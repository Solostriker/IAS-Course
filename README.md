...
1. Clone the repository using `git clone https://github.com/linguisticsjusticeleague/CurriculumBuilder`
2. Install the repositories (make sure you have Node.js installed) using `npm i` in the directory of CurriculumBuilder folder generated by the `git clone` command
3. Run the program using `npm run dev`
    * There should be a "localhost" link when you run this command. Copy that link and paste it into any browser 
    * At first, you have to wait for a while for the website to compile. Then it should appear. ~3-5 seconds 

## Accounts
* There are two accounts already created for you to test
    * Student account: **username**: `student`, **password**: `hellothere`
    * Organizer account: **username**: `organizer`, **password**: `hellothere`
...
* *Note*: don't worry about completing landing page. The plan is it will be replaced by the [Linguistic Justice League site](https://linguisticsjusticeleague.org/) once we are done with deveopment.
